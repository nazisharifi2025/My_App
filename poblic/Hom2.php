            ...
            <!-- img ended -->
             <!-- comment stert -->
              <div>
                <?php
                $postId =$row["id"]
                $CommentDastor = "SELECT COUNT(*) AS totalComments FROM Comment WHERE Post_id ='$postId'";
                $narijaComment = $connect->query($CommentDastor);
                $totalComment = $narijaComment->fetch_assoc()['totalComments']
                ?>
                <h1><?php echo $totalComment?></h1>
            <a href=<?php echo "commint.php?id=".$row['id'] ?> class="w-[100%] px-6 text-right mx-auto"><span class="material-symbols-outlined">
comment
            </span></a> 
            </div>
            <!-- comment-ended -->
